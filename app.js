...
function authenticationToken(request, response, next) {
  ...
  app.get("/states/",authenticationToken,async(request,response)=>{
      ...
      response.send(statesArray.map((eachState)=>convertStateObjectToResponseObject(eachState)));
  });
  ...
  app.get("/district/:districtId/",authenticationToken,async(request,response)=>{
      ...
      const getDistrictQuery = `SELECT * FROM state WHERE district_id =${districtId};`;
      const district = await database.all(getDistrictQuery);
      response.send(convertDistrictObjectToResponseObject(district));
  });

  app.post("/district/:districtId/",authenticationToken,async(request,response)=>{
  ...
  app.get("/states/:stateId/stats/", authenticationToken, async (request, response) => {
    ...
    const stateArray = await db.get(getStateStatsQuery);
    response.send({totalCases:stats["SUM(cases)"],totalCures:stats["SUM(cured)"],totalActive:stats["SUM(active)"],totalDeaths:stats["SUM(deaths)"],});
    });
